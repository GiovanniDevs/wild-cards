...
(() => {
  ...
  function getUnmatchedFaceDownCards() {
    ...
    console.log(
      "Matched cards:",
      gridEl.querySelectorAll(".card.matched").length
    );
    console.log(
      "Flipped cards:",
      gridEl.querySelectorAll(".card.flipped").length
    );
  ...
  function selectCardsToSwap(eligibleCards) {
    if (eligibleCards.length < 2) {
      console.log(
        "selectCardsToSwap: Not enough eligible cards (< 2), skipping swap"
      );
      return null;
    }
    ...
    console.log(
      "Swapping:",
      selectedCards[0].dataset.face,
      "↔",
      selectedCards[1].dataset.face
    );
  ...
  function getCardContainer(card) {
    ...
    if (container) {
      console.log(
        "getCardContainer: Found container for card",
        card.dataset.face
      );
    } else {
      console.warn(
        "getCardContainer: No .col-4 container found for card",
        card.dataset.face
      );
    }
  ...
  function swapCardPositions(card1, card2) {
    ...
    const firstCenterX1 =
      firstRect1.left - gridRect.left + firstRect1.width / 2;
    const firstCenterY1 = firstRect1.top - gridRect.top + firstRect1.height / 2;
    const firstCenterX2 =
      firstRect2.left - gridRect.left + firstRect2.width / 2;
    ...
    console.log(
      ...
      gridRect.left.toFixed(2),
      gridRect.top.toFixed(2)
    );
    console.log(
      "Card 1 initial center (relative to grid):",
      firstCenterX1.toFixed(2),
      firstCenterY1.toFixed(2)
    );
    console.log(
      "Card 2 initial center (relative to grid):",
      firstCenterX2.toFixed(2),
      firstCenterY2.toFixed(2)
    );
  ...
  function startSwapInterval() {
    ...
    console.log(
      "Swap interval started - will swap cards every",
      SWAP_INTERVAL_MS / 1000,
      "seconds"
    );
  }
