
# Wild Cards

This project was a group of 5 task during the lesins.
The project representing a simple memory game.
Featuring a time limitations and difficulties.
## Deployment link

```bash
  https://giovannidevs.github.io/wild-cards/
```
## Wireframes

![App Screenshot](https://external-content.duckduckgo.com/iu/?u=https%3A%2F%2Fimg.jagranjosh.com%2Fimages%2F2023%2FJanuary%2F1712023%2Fragdoll-top-10-cat-breeds.jpg&f=1&nofb=1&ipt=80e8dd07b5dc6855265bfc2b578bdc6b3d5b18cb0108dfa61eaabb49dfd27b59)

![App Screenshot](https://raw.githubusercontent.com/test.png)
## Color Scheme

| Color             | Hex                                                                |
| ----------------- | ------------------------------------------------------------------ |
| Example Color | ![#735F50](https://via.placeholder.com/10/0a192f?text=+) #735F50 |
| Example Color | ![#eed8b9](https://via.placeholder.com/10/f8f8f8?text=+) #eed8b9 |
| Example Color | ![#566F96](https://via.placeholder.com/10/00b48a?text=+) #566F96 |
| Example Color | ![#8d99ce](https://via.placeholder.com/10/00b48a?text=+) #8d99ce |
| Example Color | ![#F7F7F7](https://via.placeholder.com/10/00b48a?text=+) #F7F7F7 |

This colors representing:
- Trust
- Loyalty
- Support
- Purity
## Features

The site uses Bootstrap components. Key UI features include:

- Information button about the game rules.
- 3 Difficulty level (Easy, Normal, Hard).
- A timer countdown.
- Footer with social media and credits.
# Tests resolts

## Google Lighthouse
![App Screenshot](https://external-content.duckduckgo.com/iu/?u=https%3A%2F%2Fimg.jagranjosh.com%2Fimages%2F2023%2FJanuary%2F1712023%2Fragdoll-top-10-cat-breeds.jpg&f=1&nofb=1&ipt=80e8dd07b5dc6855265bfc2b578bdc6b3d5b18cb0108dfa61eaabb49dfd27b59)

## CSS Validation Service
![App Screenshot](https://raw.githubusercontent.com/test.png)

## Markup Validation Service
![App Screenshot](https://raw.githubusercontent.com/test.png)

## Bugs

!! Information explained !!

- Test 01
- Test 02

### Known Issues

!! Information explained !!

- Test 01
- Test 02

## Credits

### Member who worked on this project.

**Terence Reilly** Wireframe,

**Giovanni Devs** Github Master,

**Iain Kirkham** 

**Viktor Goönczöl** HTML (Header, Footer), CSS, Cards style, Readme

**Valentino Farias** 